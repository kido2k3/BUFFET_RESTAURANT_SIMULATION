...
class Customer:
    ...
    def set_waiting_time(self, waiting_time):
        ...
        if waiting_time > TIME_CAN_WAIT:
            ...
            print(
                f"Customer {self.id} waited for {self.waiting_time:7f} and decreased the rating by {decrease}")
    ...
    def pick_food_to_queue(self):
        indices_with_one = [index for index,
                            value in enumerate(self.taking_food) if value == 1]
...
class Restaurant:
    ...
    def serve_appetizer(self, env, consumer):
        with self.server_appetizer.request() as req:
            ...
            consumer.arrival_time = env.now
            print(
                f'Customer {consumer.id:3} entered appetizer at {env.now:7.3f}')
    ...
    def serve_mainCourses(self, env, consumer):
        with self.server_mainCourses.request() as req:
            ...
            consumer.arrival_time = env.now
            print(
                f'Customer {consumer.id:3} entered mainCourse at {env.now:7.3f}')
    ...
    def serve_desserts(self, env, consumer):
        with self.server_desserts.request() as req:
            ...
            consumer.arrival_time = env.now
            print(
                f'Customer {consumer.id:3} entered desserts at {env.now:7.3f}')
    ...
    def serve_seat_eat(self, env, consumer):
        with self.server_Seat_eat.request() as req:
            ...
            # consumer.set_waiting_time(env.now - consumer.arrival_time)
            print(
                f'Customer {consumer.id:3} entered seat to eat at {env.now:7.3f}')
    ...
    def leave_n_rate(self, env, consumer):
        print(
            f"Customer {consumer.id:3} left the restaurant at {env.now:7.3f} and rated {consumer.rating} stars")
